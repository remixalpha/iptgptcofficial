...
export default function AboutUS() {
  ...
  return (
    <motion.div
      ref={ref}
      className="mx-auto max-w-2xl flex flex-col-reverse items-center gap-x-20 px-10 mb-10 bg-white md:max-w-7xl lg:grid lg:grid-cols-2 lg:max-w-7xl xl:grid xl:grid-cols-2 xl:max-w-7xl"
      variants={container}
      initial="hidden"
    ...
    >
      <div className="group space-y-10 border rounded-primary p-8">
        ...
        <Link to="/about" className="flex flex-row">
          ...
          <motion.div
            className="relative top-4 right-8 w-5 h-5 text-white cursor-pointer group-hover:right-1 transition-all duration-300"
            variants={item}
          >
            <FiChevronRight />
          </motion.div>
